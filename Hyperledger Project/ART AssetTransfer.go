...
func main() {
	...
	log.Println("--> Submit Transaction: CreateAsset, creates new asset with arguments")
	result, err = contract.SubmitTransaction("CreateAsset", "asset7","Guacamole", "Sari Shryack", "2021", "Oil on wood", "8 by 8 in", "Painting of brightly colored guacamole", "80 USD", "50 USD", "2021", "Private Collection", "Sari Shryack")
	if err != nil {
		log.Fatalf("Failed to Submit transaction: %v", err)
	}
	log.Println(string(result))
	...
	result, err = contract.EvaluateTransaction("ReadAsset", "asset7")
	if err != nil {
	...
	_, err = contract.SubmitTransaction("TransferAsset", "asset1", "Tom", "May 1 2021", "1234567 USD")
	if err != nil {
		log.Fatalf("Failed to Submit transaction: %v", err)
	}
